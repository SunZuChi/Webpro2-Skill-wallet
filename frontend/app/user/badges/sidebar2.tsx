...
import {
  LayoutDashboard, Medal, FolderOpen, FileUp,
  Settings, AlignLeft, LogOut, X,
} from 'lucide-react';
...
export const Sidebar2 = ({ isCollapsed, onToggle }: { isCollapsed: boolean; onToggle: () => void }) => {
  ...
  const navItems = [
    ...
    { href: '/user/badges', icon: <Medal size={20} />, label: 'My Badges', active: true },
    { href: '/user/skill-hub', icon: <FolderOpen size={20} />, label: 'Skill Hub & CV', active: false },
    { href: '/user/ex', icon: <FileUp size={20} />, label: 'Export Resume', active: false },
  ];
    ...
    <>
            ...
            <nav className="flex flex-col w-full px-4 gap-2">
              {navItems.map(item => (
                <Link key={item.href} href={item.href} onClick={() => setDrawerOpen(false)}>
                  <div className={`flex items-center gap-4 px-4 py-3.5 rounded-xl text-sm cursor-pointer transition-all relative overflow-hidden ${item.active ? 'bg-white/5 text-white border border-white/10 font-bold' : 'text-slate-400 hover:bg-white/5 hover:text-white font-medium'}`}>
              ...
              <div className="relative">
                ...
                {showLogout && (
                  <div className="absolute left-0 bottom-full mb-2 w-full bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-80">
                    <button onClick={() => window.location.reload()} className="w-full flex items-center gap-3 px-4 py-3 text-rose-500 hover:bg-rose-500/10 text-xs font-bold transition-colors cursor-pointer"><LogOut size={16} /> Logout</button>
                  </div>
                )}
              </div>
      ...
      <aside className={`hidden md:flex ${isCollapsed ? 'w-20' : 'w-65'} h-full bg-[#0a0a0a] border-r border-white/5 flex-col py-8 shrink-0 z-50 transition-all duration-300 ease-in-out`}>
        <div className={`px-6 mb-12 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} w-full`}>
          <button onClick={onToggle} className="p-2 hover:bg-white/5 rounded-lg text-slate-400 transition-colors focus:outline-none cursor-pointer"><AlignLeft size={24} /></button>
          {!isCollapsed && <span className="text-xl font-bold tracking-tighter select-none animate-in fade-in duration-300"><span className="text-[#ff4f40]">Ip</span>&s</span>}
        </div>
        <nav className="flex flex-col w-full px-4 gap-2">
          {navItems.map(item => (
            <Link key={item.href} href={item.href}>
              <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-4'} px-4 py-3.5 rounded-xl text-sm cursor-pointer transition-all relative overflow-hidden group ${item.active ? 'bg-white/5 text-white border border-white/10 font-bold' : 'text-slate-400 hover:bg-white/5 font-medium'}`}>
                {item.active && !isCollapsed && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-5 bg-[#ff4f40] rounded-r-full shadow-[2px_0_10px_rgba(255,79,64,0.5)]" />}
          ...
          <div className="relative">
            ...
            {showLogout && (
              <div className={`absolute ${isCollapsed ? 'left-16' : 'left-0'} bottom-full mb-2 w-full min-w-30 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 duration-200 z-60`}>
                <button onClick={() => window.location.reload()} className="cursor-pointer w-full flex items-center gap-3 px-4 py-3 text-rose-500 hover:bg-rose-500/10 text-sm font-bold transition-colors"><LogOut size={16} /> Logout</button>
              </div>
            )}
          </div>
          <div className="h-px bg-white/5 mx-2" />
