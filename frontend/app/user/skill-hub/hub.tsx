...
export default function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      default:
        return <div className={`${CONTAINER_CLASS} py-20 text-center opacity-20`}><h2 className="text-2xl font-bold uppercase tracking-widest text-white">Section Coming Soon</h2></div>;
    }
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#050505] text-white font-lineseed selection:bg-[#ff4f40]/30 relative">
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 overflow-y-auto flex flex-col relative transition-all duration-300 pt-14 md:pt-0">
      ...
      <style dangerouslySetInnerHTML={{ __html: `.custom-scrollbar::-webkit-scrollbar{width:4px}.custom-scrollbar::-webkit-scrollbar-track{background:transparent}.custom-scrollbar::-webkit-scrollbar-thumb{background:#27272a;border-radius:10px}` }} />
    </div>
  );
}
    ...
    <div className={`${CONTAINER_CLASS} py-8 md:py-10 space-y-14 animate-in fade-in duration-700`}>
      ...
      <section>
        <div className="bg-[#0f0f11] border border-white/5 rounded-[2.5rem] overflow-hidden relative shadow-2xl group">
          <div className="h-32 md:h-48 bg-linear-to-r from-[#1a1a1a] via-[#ff4f40]/10 to-[#1a1a1a]" />
          <div className="px-6 md:px-12 pb-10 relative">
            <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-6 -mt-12 md:-mt-16 mb-8">
              ...
              <div className="flex-1 space-y-2 md:pb-2 text-left">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-white">Yosapart Raúl</h2>
                  <button onClick={onEditProfile} className="cursor-pointer p-3 bg-white/5 hover:bg-[#ff4f40] hover:text-white rounded-2xl transition-all text-slate-400">
                    <Edit3 size={18} />
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-3 md:gap-6 pt-2 text-slate-500 text-xs sm:text-sm font-medium">
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
          {MOCK_PROJECTS.map(project => (
            <div key={project.id} onClick={() => onPreviewProject(project)} className="bg-[#0f0f11] border border-white/5 rounded-[2.2rem] flex flex-col min-h-90 hover:border-[#ff4f40]/30 transition-all cursor-pointer group relative overflow-hidden shadow-xl">
          ...
          <div onClick={onAddProject} className="bg-[#050505] border-2 border-dashed border-white/5 rounded-[2.2rem] p-8 flex flex-col items-center justify-center text-center group cursor-pointer hover:border-[#ff4f40]/40 transition-all min-h-90 shadow-2xl">
            ...
            <p className="text-[11px] text-slate-500 uppercase font-black tracking-widest max-w-50 leading-relaxed">Upload and showcase your latest achievements.</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          {displayBadges.length > 0 ? displayBadges.map((badge) => (
            <div key={badge.id} className="bg-[#0f0f11] border border-white/5 shadow-2xl rounded-[2.2rem] p-8 flex flex-col min-h-85 group transition-all hover:border-[#ff4f40]/40 relative animate-in zoom-in-95 duration-500">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onAddExperience} className="cursor-pointer w-10 h-10 rounded-full bg-[#1a1a1a] hover:bg-[#ff4f40] transition-all flex items-center justify-center text-white shadow-lg transform active:scale-90"><Plus size={20} /></button>
          </div>
          <div className="bg-[#0f0f11] border border-white/5 rounded-[2.5rem] p-8 min-h-70 flex items-center justify-center group cursor-pointer hover:border-white/10 transition-all shadow-xl">
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onAddEducation} className="cursor-pointer w-10 h-10 rounded-full bg-[#1a1a1a] hover:bg-[#ff4f40] transition-all flex items-center justify-center text-white shadow-lg transform active:scale-90"><Plus size={20} /></button>
          </div>
          <div className="bg-[#0f0f11] border border-white/5 rounded-[2.5rem] p-8 min-h-70 flex flex-col justify-between hover:border-[#ff4f40]/20 transition-all group shadow-xl relative overflow-hidden">
            <div className="space-y-4 text-left">
              <div className="flex items-center justify-between">
            ...
            <div className="pt-6 border-t border-white/5 text-right">
              <button onClick={onAddEducation} className="text-[10px] font-black text-slate-500 hover:text-[#ff4f40] uppercase tracking-widest flex items-center gap-1 transition-all ml-auto">EDIT <ChevronRight size={12} /></button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-250 flex items-center justify-center p-4 sm:p-6 md:p-10 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-5xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300 text-left">
        <div className="h-48 sm:h-64 md:h-96 w-full relative">
          ...
          <button onClick={onClose} className="absolute top-6 right-6 md:top-8 md:right-8 p-3 bg-black/50 backdrop-blur-md border border-white/10 rounded-full text-white hover:bg-[#ff4f40] transition-all z-50 shadow-xl"><X size={22} className="group-hover:rotate-90 transition-transform duration-300" /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-8 sm:p-10 md:p-14 custom-scrollbar -mt-16 md:-mt-20 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 md:mb-12">
            <div className="space-y-4">
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white leading-tight">{project.title}</h2>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-12">
            <div className="lg:col-span-2 space-y-8">
              <div className="space-y-4"><h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-3">Project Description <div className="h-px flex-1 bg-white/5" /></h3><p className="text-base md:text-lg text-slate-300 leading-relaxed font-light">{project.longDesc}</p></div>
            </div>
            <div className="space-y-8"><div className="bg-[#121214] border border-white/5 rounded-3xl p-8 space-y-6"><h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">Metadata</h4><div className="space-y-4"><div className="flex justify-between items-center"><span className="text-slate-500 text-sm">Status</span><span className="text-emerald-500 font-bold text-sm">Completed</span></div><div className="flex justify-between items-center"><span className="text-slate-500 text-sm">Year</span><span className="text-white font-bold text-sm">2026</span></div></div><button onClick={onEdit} className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl transition-all text-sm font-bold flex items-center justify-center gap-2 text-white"><Edit3 size={16} /> Edit Details</button></div></div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-300 flex items-center justify-center p-4 sm:p-6 md:p-10 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-4xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300 text-left">
        <div className="p-6 sm:p-10 border-b border-white/5 bg-[#0f0f11] flex justify-between items-center">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-bold text-white">{initialData ? 'Edit Project' : 'New Project Submission'}</h2>
            <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Showcase your technical legacy.</p>
          </div>
          <button onClick={onClose} className="cursor-pointer p-2 hover:bg-white/5 rounded-full text-slate-500 transition-colors"><X size={28} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 md:space-y-12 custom-scrollbar">
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <textarea placeholder="Description..." defaultValue={initialData?.longDesc} className="w-full bg-black/50 border border-white/5 rounded-3xl p-8 text-sm min-h-45 outline-none focus:border-[#ff4f40]/50 transition-all italic font-light leading-relaxed placeholder:text-slate-800 text-white shadow-inner" />
            </div>
          </section>
        </div>
        <div className="p-6 md:p-8 border-t border-white/5 bg-[#0f0f11] flex justify-end gap-4 md:gap-6">
          <button onClick={onClose} className="px-6 md:px-8 py-3 text-slate-500 font-bold uppercase tracking-widest text-[10px] hover:text-white transition-colors">Discard</button>
          <button className="bg-[#ff4f40] text-white font-extrabold px-8 md:px-12 py-4 rounded-3xl shadow-xl uppercase tracking-widest text-[10px] transform active:scale-95">Confirm Submission</button>
        </div>
      </div>
    </div>
  );
          ...
          {MOCK_BADGES.map(badge => {
            ...
            return (
              <div key={badge.id} onClick={() => !isFull && togglePin(badge.id)} className={`p-6 md:p-8 rounded-[2.5rem] border transition-all relative group cursor-pointer ${isPinned ? 'bg-[#ff4f40]/5 border-[#ff4f40]/40 shadow-2xl' : isFull ? 'opacity-30 grayscale cursor-not-allowed border-white/5' : 'bg-[#121214] border-white/5 hover:border-white/10'}`}>
                ...
                <h4 className="font-bold text-lg md:text-xl mb-2 text-white">{badge.title}</h4>
...
const TimelineEditModal = ({ isOpen, onClose, type }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
      <div className="absolute inset-0 bg-black/95 backdrop-blur-md" onClick={onClose} />
      <div className="relative w-full max-w-xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] shadow-2xl p-8 md:p-10 space-y-8 animate-in zoom-in-95 text-left">
        <div className="flex justify-between items-center"><h2 className="text-xl md:text-2xl font-bold flex items-center gap-3 capitalize text-white">{isEdu ? <GraduationCap className="text-[#ff4f40]" /> : <Briefcase className="text-[#ff4f40]" />} Edit {type}</h2><button onClick={onClose} className="cursor-pointer p-2 hover:bg-white/5 rounded-full text-slate-500"><X size={20} /></button></div>
        ...
        <div className="flex justify-end gap-4 md:gap-6 pt-6 border-t border-white/5"><button onClick={onClose} className="cursor-pointer px-6 py-3 text-slate-500 font-bold uppercase tracking-widest text-[10px] hover:text-white transition-colors">Cancel</button><button className="cursor-pointer bg-white text-black font-extrabold px-8 md:px-10 py-4 rounded-3xl shadow-xl uppercase tracking-widest text-[10px]">Save Changes</button></div>
      </div>
    </div>
  );
};

// ── PROFILE EDIT MODAL — เพิ่ม photo upload ──
      ...
      <div className="relative w-full max-w-xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] shadow-2xl p-8 md:p-10 space-y-8 animate-in zoom-in-95 text-left max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <button
            ...
            className="text-[10px] font-black text-slate-500 hover:text-[#ff4f40] uppercase tracking-widest transition-colors flex items-center gap-1.5"
          >
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <input type="text" defaultValue="Yosapart Raúl" className="w-full bg-black border border-white/5 rounded-2xl px-6 py-4 text-sm focus:border-[#ff4f40]/50 outline-none text-white shadow-inner" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Headline</label>
            <input type="text" defaultValue="Aspiring Full-Stack Developer" className="w-full bg-black border border-white/5 rounded-2xl px-6 py-4 text-sm focus:border-[#ff4f40]/50 outline-none text-white shadow-inner" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Bio</label>
            <textarea defaultValue="Senior Applied Computer Science student..." className="w-full bg-black border border-white/5 rounded-2xl p-6 text-sm min-h-30 focus:border-[#ff4f40]/50 outline-none text-white shadow-inner" />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-4 md:gap-6 pt-6 border-t border-white/5">
          ...
          <button className="cursor-pointer bg-white text-black font-extrabold px-8 md:px-10 py-4 rounded-3xl shadow-xl uppercase tracking-widest text-[10px]">Update Profile</button>
        </div>
      </div>
    </div>
  );
