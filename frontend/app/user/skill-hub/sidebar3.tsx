...
export const Sidebar3 = ({ isCollapsed, onToggle }: { isCollapsed: boolean; onToggle: () => void }) => {
  ...
  const navItems = [
    ...
    { href: '/user/skill-hub', icon: <FolderOpen size={20} />, label: 'Skill Hub & CV', active: true },
          ...
          <div className="relative">
            ...
            {showLogout && (
              <div className={`absolute ${isCollapsed ? 'left-16' : 'left-0'} bottom-full mb-2 w-full min-w-30 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 duration-200 z-60`}>
                <button onClick={() => window.location.reload()} className="cursor-pointer w-full flex items-center gap-3 px-4 py-3 text-rose-500 hover:bg-rose-500/10 text-sm font-bold transition-colors">
                  <LogOut size={16} /> Logout
                </button>
              </div>
            )}
